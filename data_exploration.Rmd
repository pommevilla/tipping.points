...
```{r test}
total_info <- read.table("data/raw/arg_all/arg_datasets.tsv", sep = "\t", header = TRUE, fill = TRUE )
colnames(total_info)[c(9, 10, 12)] <- c("environment_package", "seq_type", "mean_seq_length")
head(total_info)
```
```{r biome_info.1}
biome_info_2 <- table(unlist(total_info$biome))
ggplot(as.data.frame(biome_info_2[biome_info_2 > 50]), aes(x = reorder(Var1, Freq), y = Freq)) + geom_bar(stat = "identity") +
...
ggplot(total_info, aes(x = factor(biome), y = avg.nbsp.seq.nbsp.length)) + stat_summary(fun.y = "mean", geom = "bar")
